...
| 6 | MA stack | `!hasMaStack` | Theo pha: Favorable=`Full` / Neutral=`Medium` / Unfavorable=`Loose` (`MaStack.*Mode`) |
| 7 | TT khó + RS top | `Unfavorable && (RS percentile < MinRsPercentileForUnfavorable \|\| rs5 ≤ 0)` | **80** percentile + rs5 > 0 |
...
**Early Recovery Radar:** mã Loose (Close>MA20, MA20 slope≥0) nhưng rớt RS percentile/rs5 → `GET /api/v1/early-recovery` (không vào Top).

---

## 6. MA stack (`HasBullishMaStack`)

`MaStack.Enabled: true` — `Enabled=false` → Off mọi pha.

| Pha | Mode (config) | Rule |
|-----|---------------|------|
| Favorable | `Full` | MA20>MA50>MA100>MA200; thiếu phiên → Medium / Loose (tầng) |
| Neutral | `Medium` | MA20>MA50; `<MinSessionsForMa50` → MA20 ≥ Close×0.97 |
| Unfavorable | `Loose` | Close > MA20 **và** MA20 slope ≥ 0 (so 3 phiên) |

RS percentile: xếp hạng `rs5` trên universe đủ thanh khoản+history (`BuildContext`).
...
2. **MA stack theo pha** (Favorable Full / Neutral Medium / Unfavorable Loose+RS top)
...
| Nới MA theo pha | `MaStack.UnfavorableMode` / `NeutralMode` / `FavorableMode` (`Loose`/`Medium`/`Full`) |
| Bỏ MA stack | `MaStack.Enabled: false` — Off mọi pha |
