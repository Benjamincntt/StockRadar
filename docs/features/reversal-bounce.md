# Reversal Bounce — Đặc tả tính năng tìm đáy & đánh sóng hồi

> **Trạng thái:** Spec đã thống nhất về mặt thiết kế (qua các vòng phản biện). **Chưa được audit codebase** và **chưa code**.
>
> Đối tượng đọc: dev maintainer StockRadar. Tài liệu này *chỉ mô tả* tính năng; mọi chi tiết kiến trúc nội bộ phải được đối chiếu lại với codebase thật ở Phase 0A trước khi triển khai.
...
**Mục tiêu:** xác minh giả định trước khi viết code. Xem file hướng dẫn riêng `docs/features/reversal-bounce-phase0a-audit-instructions.md`.
...
- `docs/system-architecture.md` — kiến trúc tổng thể StockRadar.
- `docs/opportunity-scan-rules.md` — quy tắc Top strict & BuyDecision.
- `docs/base-price-engine.md` — BaseQualityEvaluator (VC/Darvas/Spring).
- `docs/telegram-vip-alerts-flow.md` — luồng alert Master Alert (T+3).
- `docs/features/reversal-bounce-phase0a-audit-instructions.md` — hướng dẫn cho model nhỏ hơn thực thi Phase 0A.