...
- **Lịch sử lệnh T+2.5**: `GET /api/v1/performance/alert-history?kind=buy` (+ `from`/`to`); trends `GET /api/v1/performance/alert-history/trends?period=week|month|quarter` — chỉ Mua điểm 1/2; mobile tab **Hiệu quả** `/performance`
